...
namespace details
{
	...
	static inline auto deref_noexcept(It& it) -> decltype(*it)
	{
		return *it;
	}
...
private:
	...
	struct Block
	{
		...
		inline T* operator[](index_t idx) { return static_cast<T*>(static_cast<void*>(elements)) + static_cast<size_t>(idx & static_cast<index_t>(BLOCK_SIZE - 1)); }
		inline T const* operator[](index_t idx) const { return static_cast<T const*>(static_cast<void const*>(elements)) + static_cast<size_t>(idx & static_cast<index_t>(BLOCK_SIZE - 1)); }
		
	private:
		static_assert(std::alignment_of<T>::value <= sizeof(T), "The queue does not support types with an alignment greater than their size at this time");
		alignas(alignof(T)) char elements[sizeof(T) * BLOCK_SIZE];
	public:
		Block* next;
	...
	friend void moodycamel::swap(typename ConcurrentQueue<XT, XTraits>::ImplicitProducerKVP&, typename ConcurrentQueue<XT, XTraits>::ImplicitProducerKVP&) ;
